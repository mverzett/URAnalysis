...
class BasePlotter(object):
	...
	def add_cms_watermark(self, epoch, preliminary=True, lumi=None, lumiformat='%0.1f'):#, lumiformat='%0.1f'):
			''' Add the CMS blurb '''
			self.pad.cd()
			latex = ROOT.TLatex()
			...
			latex.SetTextAlign(11)
			self.keep.append(latex.DrawLatex(0.22,0.82, "#bf{CMS}"))
			if preliminary:
				self.keep.append(latex.DrawLatex(0.22,0.76, "#it{Preliminary}"))
			latex.SetTextAlign(31)
			latex.SetTextSize(0.04)
			lumitext = (lumiformat + " fb^{-1} ") % (lumi/1000.) if lumi is not None else ""
			self.keep.append(latex.DrawLatex(0.96,0.92, lumitext+epoch))
	...
	def save(self, filename=None, verbose=False, **inkwargs):
			...
			elif 'watermark' in self.defaults:
				self.add_cms_watermark(*self.defaults['watermark'])
			#
			if filename is None:
