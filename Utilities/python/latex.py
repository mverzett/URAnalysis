...
def format_with_asym_error(val, errup, errdw):
...
def format_roovar(rvar, asymm_err=False):
   value = rvar.value
   if asymm_err:
      eu, ed = rvar.error if hasattr(rvar.error, '__len__') else (rvar.error, -1*rvar.error)
      return format_with_asym_error(value, eu, ed) 
   else:
