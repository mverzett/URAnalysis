...
class CTagEfficiency(PhysicsModel):
	...
	def doParametersOfInterest(self):
		...
		if self.opts.fitLightEff:
			...
			#self.modelBuilder.doVar('lightSF[1.5,1.2,1.8]')
	...
	def getYieldScale(self,bin,process):
		def constrain(sfname, effname):
			'limits the SF range to avoid an eff > 1'
			sf = self.modelBuilder.out.arg(sfname)
			eff = self.modelBuilder.out.arg(effname).getMax()
			limit = 1./eff
			if sf.getMax() > limit:
				print 'limiting', sfname, 'to', limit, 'to avoid unphysical values'
				sf.setMax(limit)				
		...
		if self.paganini:
			...
			self.paganini = False
			constrain('charmSF', 'mc_lead_charm_eff')
			constrain('charmSF', 'mc_sub_charm_eff' )
			if self.opts.fitLightEff:
				constrain('lightSF', 'mc_lead_light_eff')
				constrain('lightSF', 'mc_sub_light_eff' )
